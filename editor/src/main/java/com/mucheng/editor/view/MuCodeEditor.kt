...
import com.mucheng.text.model.standard.TextModelWorker
import java.io.*
...
//        val offsetX = styleManager.painters.lineNumberPainter.measureText(visibleLineEnd.toString()) + margin * 4 + dividingLineWidth
    ...
    open fun open(reader: Reader, onSaveProgress: () -> Boolean = { true }): MuCodeEditor {
        post {
            languageManager.autoCompletionPanel.dismiss()
        }
        ...
        TextModelWorker.read(textModel, reader, onSaveProgress)
    ...
    open fun open(
        ...
        onSaveProgress: () -> Boolean = { true }
    ): MuCodeEditor {
        return open(inputStream.reader(), onSaveProgress)
    }

    @Synchronized
    open fun save(writer: Writer, onSaveProgress: () -> Boolean = { true }): MuCodeEditor {
        TextModelWorker.save(textModel, writer, onSaveProgress)
        return this
    }
